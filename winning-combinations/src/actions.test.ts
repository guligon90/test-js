import { mapSymbolsToIndexes, extractWinningCombinations } from "./actions";

test.each([
  [
    [1, 6, 6, 7, 2, 3],
    { '1': [ 0 ], '2': [ 4 ], '3': [ 5 ], '6': [ 1, 2 ], '7': [ 3 ] }
  ],
  [
    [1, 6, 6, 7, 2, 2],
    { '1': [ 0 ], '2': [ 4, 5 ], '6': [ 1, 2 ], '7': [ 3 ] }
  ],
  [
    [1, 2, 6, 6, 6],
    { '1': [ 0 ], '2': [ 1 ], '6': [ 2, 3, 4 ] }
  ],
  [
    [3, 3, 3, 8, 6, 3],
    { '3': [ 0, 1, 2, 5 ], '6': [ 4 ], '8': [ 3 ] }
  ],
  [
    [3, 3, 3, 8, 8, 8],
    { '3': [ 0, 1, 2 ], '8': [ 3, 4, 5 ] }
  ],
  [
    [3, 4, 3, 3, 3, 3],
    { '3': [ 0, 2, 3, 4, 5 ], '4': [ 1 ] }
  ],
  [
    [9, 9, 5, 9, 9],
    { '5': [ 2 ], '9': [ 0, 1, 3, 4 ] }
  ],
  [
    [9, 5, 5, 9, 9],
    { '5': [ 1, 2 ], '9': [ 0, 3, 4 ] }
  ],
  [
    [9, 5, 9, 5, 9],
    { '5': [ 1, 3 ], '9': [ 0, 2, 4 ] }
  ],
  [
    [9, 5, 9, 5, 9],
    { '5': [ 1, 3 ], '9': [ 0, 2, 4 ] }
  ],
  [
    [5, 9, 5, 9, 5],
    { '5': [ 0, 2, 4 ], '9': [ 1, 3 ] }
  ],
  [
    [6, 6, 3, 0, 6],
    { '0': [ 3 ], '3': [ 2 ], '6': [ 0, 1, 4 ] }
  ],
  [
    [1, 2, 0, 0, 3, 3],
    { '0': [ 2, 3 ], '1': [ 0 ], '2': [ 1 ], '3': [ 4, 5 ] }
  ],
  [
    [1, 1, 0, 0, 3, 3],
    { '0': [ 2, 3 ], '1': [ 0, 1 ], '3': [ 4, 5 ] }
  ],
  [
    [0, 0, 0, 3, 3, 3],
    { '0': [ 0, 1, 2 ], '3': [ 3, 4, 5 ] }
  ],
  [
    [0, 0, 2, 3, 3, 3],
    { '0': [ 0, 1 ], '2': [ 2 ], '3': [ 3, 4, 5 ] }
  ],
  [
    [2, 0, 0, 3, 3, 3],
    { '0': [ 1, 2 ], '2': [ 0 ], '3': [ 3, 4, 5 ] }
  ],
  [
    [4, 4, 6, 0, 2, 2],
    { '0': [ 3 ], '2': [ 4, 5 ], '4': [ 0, 1 ], '6': [ 2 ] }
  ],
  [
    [3, 5, 8, 5, 5, 0],
    { '0': [ 5 ], '3': [ 0 ], '5': [ 1, 3, 4 ], '8': [ 2 ] }
  ],
  [
    [3, 0, 3, 4, 4, 0],
    { '0': [ 1, 5 ], '3': [ 0, 2 ], '4': [ 3, 4 ] }
  ],
  [
    [0, 8, 6, 8, 8],
    { '0': [ 0 ], '6': [ 2 ], '8': [ 1, 3, 4 ] }
  ],
  [
    [8, 8, 6, 8, 0],
    { '0': [ 4 ], '6': [ 2 ], '8': [ 0, 1, 3 ] }
  ],
  [
    [0, 0, 0, 0, 8],
    { '0': [ 0, 1, 2, 3 ], '8': [ 4 ] }
  ],
  [
    [8, 0, 0, 0, 0],
    { '0': [ 1, 2, 3, 4 ], '8': [ 0 ] }
  ],
  [
    [0, 0, 0, 0, 0],
    { '0': [ 0, 1, 2, 3, 4 ] }
  ],
  [
    [1, 1, 2, 0, 0],
    { '0': [ 3, 4 ], '1': [ 0, 1 ], '2': [ 2 ] }
  ],
  [
    [11, 0, 0, 7, 4],
    { '0': [ 1, 2 ], '4': [ 4 ], '7': [ 3 ] }
  ],
])(`${mapSymbolsToIndexes.name} - when line is %j returns %j`, (line, expected) => {
  const received = mapSymbolsToIndexes(line);

  expect(received).toEqual(expected);
});

test.each([
  [[1, 6, 6, 7, 2, 3], []],
  [[1, 6, 6, 7, 2, 2], []],
  [[1, 2, 6, 6, 6], [[6, [2, 3, 4]]]],
  [[3, 3, 3, 8, 6, 3], [[3, [0, 1, 2]]]],
  [[3, 3, 3, 8, 8, 8], [[3, [0, 1, 2]], [8, [3, 4, 5]]]],
  [[3, 4, 3, 3, 3, 3], [[3, [2, 3, 4, 5]]]],
  // [[9, 9, 5, 9, 9], []],
  // [[9, 5, 5, 9, 9], []],
  // [[9, 5, 9, 5, 9], []],
  // [[9, 5, 9, 5, 9], []],
  // [[5, 9, 5, 9, 5], []],
  // [[6, 6, 3, 0, 6], []],
  // [[1, 2, 0, 0, 3, 3], [[2, [1, 2, 3]], [3, [2, 3, 4, 5]]]],
  // [[1, 1, 0, 0, 3, 3], [[1, [0, 1, 2, 3]], [3, [2, 3, 4, 5]]]],
  // [[0, 0, 0, 3, 3, 3], [[3, [0, 1, 2, 3, 4, 5]]]],
  // [[0, 0, 2, 3, 3, 3], [[2, [0, 1, 2]], [3, [3, 4, 5]]]],
  // [[2, 0, 0, 3, 3, 3], [[2, [0, 1, 2]], [3, [1, 2, 3, 4, 5]]]],
  // [[4, 4, 6, 0, 2, 2], [[2, [3, 4, 5]]]],
  // [[3, 5, 8, 5, 5, 0], [[5, [3, 4, 5]]]],
  // [[3, 0, 3, 4, 4, 0], [[3, [0, 1, 2]], [4, [3, 4, 5]]]],
  // [[0, 8, 6, 8, 8], []],
  // [[8, 8, 6, 8, 0], []],
  // [[0, 0, 0, 0, 8], [[8, [0, 1, 2, 3, 4]]]],
  // [[8, 0, 0, 0, 0], [[8, [0, 1, 2, 3, 4]]]],
  // [[0, 0, 0, 0, 0], [[0, [0, 1, 2, 3, 4]]]],
  // [[1, 1, 2, 0, 0], [[2, [2, 3, 4]]]],
  // [[11, 0, 0, 7, 4], [[7, [1, 2, 3]]]],
])
  (`${extractWinningCombinations.name} - when line is %j returns %j`, (line, expected) => {
    const received = extractWinningCombinations(line);

    console.log(received);

    // expect(received).toEqual(expected);
  });
